...
```
1. O que é iteração?
É uma repetição de um método dentro do Array.


2. Qual a diferença de uma function padrão e a arrow function?
//A função é um procedimento do Javascript. Ele executa uma tarefa ou faz cálculo númerico.
Arrow function é uma expressão com uma sintaxe mais curta, com algumas diferenças, como por exmeplo, ela pode ser anônima e não precisa ter chaves.

3. Como funciona o método map()? 
É um tipo de for, sendo um mapa do valor expressado, passando por cada elemento do array, fazendo uma nova chamada (callback).Com ele é possível obter um novo array. 
```
